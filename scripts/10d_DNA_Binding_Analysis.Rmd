...
## Look at alignments of orthologs

```{r}
# get close orthologs from oak and chestnut

get_orthos <- function(ids, filestem, my.hittable=hittable, my.blast_peps=blast_peps, my.blastp_ortho=blastp_ortho, my.Qrob_hittable=Qrob_hittable, my.qrobAA=qrobAA, my.tanoak_aa=tanoak_aa, my.tanoak_dna_bind_aa=tanoak_dna_bind_aa) {
...
```{r}
gene_table <- tibble(
  id=c("augustus-1014_pilon-processed-gene-5.8",
       "augustus-542_pilon-processed-gene-26.72",
       "maker-431_pilon-augustus-gene-9.19",
       "augustus-266_pilon-processed-gene-97.305",
       "maker-74056_pilon-snap-gene-18.97",
       "maker-261_pilon-snap-gene-47.3",
       "maker-345_pilon-snap-gene-67.1",
       "augustus-353_pilon-processed-gene-14.64",
       "augustus-267_pilon-processed-gene-29.9"),
  stem=c("1014_RAS1",
         "542_RAS1",
         "431_ATHB-7",
         "266_ATHB-7",
...
walk2(gene_table$id, gene_table$stem, get_orthos)
```

## small alignment notes

1014 and 542_RAS1: Qurub orthologs aren't right (Removed)

1014: Not clear that we have the right Caden ortholog

353_HSFA2: Qrob orthlog looks to have annotation error; sequence diverges at 3' end. (Editted to truncate)


## Calculate divergence from Caden


```{r}

tryMask  <-  function(aln) { # needed because if there are no gaps then we get an error
  tryCatch(error = function(cnd) aln,
           maskGaps(aln, min.fraction=.1, min.block.width=1))
}

calcChanges <- function(mat) {
  rownames(mat) <- rownames(mat) %>%
    str_remove("[\\._].*") %>%
    str_replace("target", "Tanoak")
  
  caden <- mat["Caden",]
  
  mat <- mat[!str_detect(rownames(mat), "Caden"),]
  
  result <- apply(mat, 1, function(x) sum(x!=caden)) %>% as_tibble_row()
...
files <- dir("../output/trees", pattern = "small_orthos_aligned", full.names = TRUE) %>% 
  str_subset(pattern="1014", negate = TRUE)

changes_rel_caden <- tibble(file=files) %>%
  mutate(alignment = map(file, readAAMultipleAlignment),
         alignment = map(alignment, tryMask),
         mat = map(alignment, as.matrix),
         changes = map(mat, calcChanges),
         file=basename(file)
         )

changes_rel_caden %>% select(file, changes) %>% unnest(changes)
```
