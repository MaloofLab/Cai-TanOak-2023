...
bin_dnds_values <- function(dnds_ratio) {
  ...
  bin <- ifelse(dnds_ratio < 0.1, 5,
         ifelse(dnds_ratio >= 0.1 & dnds_ratio < 0.3, 4,
         ifelse(dnds_ratio >= 0.3 & dnds_ratio < 0.8, 3,
         ifelse(dnds_ratio >= 0.8 & dnds_ratio < 1.2, 2, 1))))
}
...
unadj_pvalues[, 1] <- 1:5 # first col is bins
...
pvalues_FDR_transposed <- unadj_pvalues %>% pivot_longer(-bin, names_to = "GO_term", values_to = "p.value") %>%
  ...
  dplyr::select(GO_term, GO_desc, `5`, `4`, `3`, `2`, `1`) %>% # reverse the order. 5 = low dn/ds bin and 1 = high dn/ds bin
  mutate(`5.FDR` = p.adjust(`5`, method = "fdr"),
...
# GO:0055114 description from GO.db is NA, looking the description online the description is
# oxidation-reduction process
# hard code description for now
FDR_filtered[which(FDR_filtered$GO_term == "GO:0055114"), "GO_term_desc"] <- "GO:0055114; oxidation-reduction process"
FDR_filtered
...
colnames(FDR_filtered) <- c("GO_term", "GO_desc", "< 0.1", "0.1 - 0.29", "0.3 - 0.79", "0.8 - 1.19", ">= 1.2", "GO_term_desc")
...
FDR_filtered %>% pivot_longer(-c(contains("GO")), names_to = "bins", values_to = "FDR") %>%
  ...
  ggplot(aes(x = factor(bins, levels = c("< 0.1", "0.1 - 0.29", "0.3 - 0.79", "0.8 - 1.19", ">= 1.2")), 
