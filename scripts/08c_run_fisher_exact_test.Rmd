...
```{r}
is_significant <- function(FDR) {
  FDR <= 0.25
}

FDR_filtered <- pvalues_FDR_transposed %>% filter(if_any(contains(".FDR"), is_significant)) %>%
...
colnames(FDR_filtered) <- c("GO_term", "GO_desc", "< 0.1", "0.1 - 0.29", "0.3 - 0.79", "0.8 - 1.19", ">= 1.2")
...
FDR_filtered %>% pivot_longer(-c(contains("GO")), names_to = "bins", values_to = "FDR") %>%
  ...
  ggplot(aes(x = factor(bins, levels = c("< 0.1", "0.1 - 0.29", "0.3 - 0.79", "0.8 - 1.19", ">= 1.2")), 
                        y = GO_term, fill = cut(FDR, c(0, 0.01, 0.05, 0.25, 1)))) + geom_tile() +
