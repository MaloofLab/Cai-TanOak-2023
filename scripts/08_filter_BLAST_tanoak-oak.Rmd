...
check names
```{r}
length(intersect(tanoak_v_oak_blastp$query_id, oak_v_tanoak_blastp$subject_id))
length(intersect(oak_v_tanoak_blastp$query_id, tanoak_v_oak_blastp$subject_id))
```
...
tanoak_v_oak_blastp_best <- tanoak_v_oak_blastp %>%
  ...
  dplyr::select(query_id, subject_id, pct_ident, len, E, nextE, E_diff, Score)
...
tanoak_blastp_recip <- left_join(tanoak_v_oak_blastp_best, oak_v_tanoak_blastp_best,
  ...
  dplyr::select(query_id, subject_id, subject_id.oak_tanoak, everything()) # rearrange columns 
...
```{r}
# load in fasta files
tanoak_fasta <- readDNAStringSet("../input/blast/TanOak.transcripts.fa.gz")
oak_fasta <- readDNAStringSet("../input/blast/Qrob_PM1N_CDS_nt_20161004.fa.gz")
...
if(! dir.exists("../output/ortho/blastp_ortholog/raw_seqs/")) dir.create("../output/ortho/blastp_ortholog/raw_seqs/", recursive = TRUE)

if(!dir.exists("../output/ortho/dcmega_ortholog/raw_seqs/")) dir.create("../output/ortho/dcmega_ortholog/raw_seqs/", recursive = TRUE)
