...
tanoak_blastp_ortho <- tanoak_blastp_ortho %>%
  ...
  mutate(filepath = paste0("../output/ortho/blastp/raw_seqs", query_id, "--", subject_id, ".fasta"))

tanoak_dcmega_ortho <- tanoak_dcmega_ortho %>%
  select(query_id, subject_id) %>%
  mutate(filepath = paste0("../output/ortho/dcmegablast/raw_seqs", query_id, "--", subject_id, ".fasta"))
...
```{r}
# generate fasta files for dcmegablast
