...
blastp_dnds[which(blastp_dnds$dnds_ratio < 0), ] # gap in the beginning of one sequence and sequences are identical
```

```{r}
# load in interpro annotations
interpro <- read_tsv("../output/PanGenome_ONT_V1_interpro.tsv",
...
blastp_dnds_annot <- left_join(blastp_dnds, interpro, by=c("tanoak"="protein_accession")) %>%
  select(tanoak, dnds_ratio, signature_description, interpro_description, GO_annotation) %>%
  distinct() %>%
...
for (i in 1:length(go_annot)) {
  if(identical(go_annot[[i]], character(0))) {
    ...
    blastp_dnds_annot_cleaned$GO_annotations[i] <- go_annot[[i]] %>% unique() %>% paste0(collapse = "|")
  }
}
