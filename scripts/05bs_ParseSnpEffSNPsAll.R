...
outfile <- str_c("Annotated_SNPeff_ALL_quant", taskID, ".csv.gz", sep="_")
...
for(i in 1:nrow(contig.splits_tab)) {

  # read it
  vcf <-  try(
    readVcf(tab, 
                  param=GRanges(
                    seqnames = contig.splits_tab$name[i],
                    ranges = IRanges(
                      start = 1,
                      end=contig.splits_tab$length[i])))
  )
