...
VRtibble <- VRtibble %>%
  mutate(sample=
           { str_replace(sample, "52.81", "SM.52.81.cl1") %>% 
...
total moderate and high:
```{r}
VRtibble %>%
  filter(impact!="LOW") %>%
  group_by(snpID) %>%
  summarize(snpID, count=n(), high=any(impact=="HIGH"), moderate=any(impact=="MODERATE"), both=all(high, moderate)) %>%
  filter(!duplicated(snpID), both==TRUE)
```
239 SNPs have both moderate and high impacts.
...
```{r}
VRtibble %>% filter(snpID=="158436_pilon:12808")
```


total genes with a a moderate or high SNP
...
genes_by_tree %>%
  ...
  geom_col() +
  ylab("Genes Homozygous for Alternate") +
  theme_bw() +
  theme(axis.text.x  =element_text(angle=90, hjust=1, vjust=0.5)) 
ggsave("~/Library/CloudStorage/GoogleDrive-jnmaloof@ucdavis.edu/Shared drives/TanOak/Figures for Paper/TanOak2A.png", dpi = 300)
```
upset plot

filtered for no duplicated samples
```{r, fig.width=10}
p <- VRtibble %>%
  filter(str_detect(sample, "XL", negate = TRUE)) %>%
  filter(str_detect(sample, "cl2", negate = TRUE)) %>%
  filter(impact!="LOW", GT=="1/1") %>%
  group_by(sample) %>%
  filter(!duplicated(GeneName)) %>%
...
upset(nsets=13, nintersects = 50, order.by = "freq", show.numbers = FALSE )
p

pdf("~/Library/CloudStorage/GoogleDrive-jnmaloof@ucdavis.edu/Shared drives/TanOak/Figures for Paper/TanOak2B_NoDups.pdf",
       width=8, height = 5)
p
dev.off()
```
...
```{r, fig.width=10}
 p <- VRtibble %>%
  ...
  table() %>%
  as.data.frame.matrix() %>%
upset(nsets=19, nintersects = 50, show.numbers = FALSE, order.by = "freq", mb.ratio = c(0.6,0.4) )

p

pdf("~/Library/CloudStorage/GoogleDrive-jnmaloof@ucdavis.edu/Shared drives/TanOak/Figures for Paper/TanOak2B_alt_all.pdf",
    width=8, height = 5)
p
dev.off()
```
...
pheno <- read_csv("../input/TanOakResistance.csv") %>%
  mutate(nameMatch=str_replace_all(Tanoak, "-","."),
           nameMatch=
           { str_replace(nameMatch, "SM.52.81", "SM.52.81.cl1") %>% 
               str_replace("LP.22.54", "SM.52.81.cl2") %>%
               str_replace("SM.54.45", "SM.74.45.cl1") %>%
               str_replace("SM.54.26", "SM.74.45.cl2") %>%
               str_replace("(^[0-9])", "SM.\\1")}
...
```{r}
VRtibble %>% pull(sample) %>% unique() %>% sort()
